...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            points = read_file(str(lidar_path), virtual=res["virtual"])  #(34720,4)

            sweep_points_list = [points]#[(34720,4)]
            sweep_times_list = [np.zeros((points.shape[0], 1))]#[(34720,1)]
            ...
            points = np.concatenate(sweep_points_list, axis=0)#(274251,4)
            times = np.concatenate(sweep_times_list, axis=0).astype(points.dtype)#(274251,1)

            res["lidar"]["points"] = points
            res["lidar"]["times"] = times
            res["lidar"]["combined"] = np.hstack([points, times])#(274251,5)
...
class LoadPointCloudAnnotations(object):
    ...
    def __call__(self, res, info):
        ...
        if res["type"] in ["NuScenesDataset"] and "gt_boxes" in info:
            gt_boxes = info["gt_boxes"].astype(np.float32) #(11,9)
