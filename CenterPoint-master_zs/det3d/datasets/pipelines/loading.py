...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            points = read_pcd(str(lidar_path))
...
class LoadPointCloudAnnotations(object):
    ...
    def __call__(self, res, info):
        annotation_labels = json.load(open(info['annotation_path'],'r'))
        gt_names, gt_boxes = [], []
        for annotation_label in annotation_labels:
            gt_box = []
            gt_box.append(annotation_label['psr']['position']['x'])
            gt_box.append(annotation_label['psr']['position']['y'])
            gt_box.append(annotation_label['psr']['position']['z'])
            gt_box.append(annotation_label['psr']['scale']['x'])
            gt_box.append(annotation_label['psr']['scale']['y'])
            gt_box.append(annotation_label['psr']['scale']['z'])
            gt_box.append(0.0)
            gt_box.append(0.0)
            gt_box.append(annotation_label['psr']['rotation']['z'])
            gt_boxes.append(np.array(gt_box))
            gt_names.append(annotation_label['obj_type'])
        gt_boxes = np.array(gt_boxes)
        gt_names = np.array(gt_names)
        

        if res["type"] in ["NuScenesDataset"] and "annotation_path" in info:
